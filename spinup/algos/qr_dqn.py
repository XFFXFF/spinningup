...
class DQNNet(object):
    ...
    def __init__(self, obs, act_n, quant_n):
        ...
        net = layers.dense(net, quant_n * act_n)
        self.quantiles = tf.reshape(net, [-1, act_n, quant_n])
        self.q_acts = tf.reduce_mean(self.quantiles, axis=2)

    def network_output(self):
        return self.quantiles, self.q_acts
...
class DQNAgent(object):
    ...
    def __init__(self,
        ...
        tau = (2 * tf.range(quant_n, dtype=tf.float32) + 1) / (2. * quant_n)

        batch_indices = tf.range(tf.to_int32(batch_size))[:, None]
        batch_indexed_acts = tf.concat([batch_indices, self.act_ph[:, None]], axis=1)
        chosen_act_quantiles = tf.gather_nd(self.quantiles, batch_indexed_acts)
        
        targ_acts = tf.argmax(self.q_targ_acts, output_type=tf.int32, axis=1)
        batch_indexed_targ_acts = tf.concat([batch_indices, targ_acts[:, None]], axis=1)
    ...
    def _create_network(self):
        with tf.variable_scope('main'):
            ...
            net = DQNNet(obs_float, self.act_n, self.quant_n)
            self.quantiles, self.q_acts = net.network_output()
        with tf.variable_scope('target'):
            next_obs_float = tf.cast(self.next_obs_ph, tf.float32) / 255.0
            net = DQNNet(next_obs_float, self.act_n, self.quant_n)
            self.targ_quantiles, self.q_targ_acts = net.network_output()
